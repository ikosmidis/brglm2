...
$Q_{1s} = {\rm diag}\{v'_1 d_1x_{1s}/v_1, \ldots, v'_n d_n x_{ns}/v_n\}$ and $Q_{2s} = Q_{1s} - {\rm diag}\{d_1x_{1s}/d'_1, \ldots, d_n x_{ns}/d'_n\}$
...
The vignette has been written and by Ioannis Kosmidis. Eugene Clovis Kenne Pagui and Nicola Sartori have contributed the section "Median bias-reducing adjusted score functions" as supporting documentation when the `brglmFit` method is used with `type = "AS_median"`.
