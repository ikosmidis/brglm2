# Copyright (C) 2026- Oliver Clark, Ioannis Kosmidis
...
#'
#' Implements trust region optimization with CG-Steihaug subproblem solver
...
#' @export
brglmFit_trustregion <- function(x, y, weights = rep(1, nobs), 
    ...
    if (!no_dispersion) {
        stop("Trust region method currently only supports binomial and Poisson families (fixed dispersion)")
    }
    ...
    if (family$family == "binomial") {
        if (any(mus > 1 - eps) || any(mus < eps)) {
            warning("brglmFit_trustregion: fitted probabilities numerically 0 or 1 occurred")
            boundary <- TRUE
        }
    }
    if (family$family == "poisson") {
        if (any(mus < eps)) {
            warning("brglmFit_trustregion: fitted rates numerically 0 occurred")
            boundary <- TRUE
        }
    }
