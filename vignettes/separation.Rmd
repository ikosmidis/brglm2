...
‘detect_separation’ and `check_infinite_estimates` will be removed
from ‘brglm2’ at version 0.8.  New versions are now maintained in the
‘detectseparation’ R package at <URL:
https://cran.r-project.org/package=detectseparation>. In order to use
the version in ‘detect_separation’ load first ‘brglm2’ and then
‘detectseparation’, i.e. ‘library(brglm2); library(detectseparation)’.
